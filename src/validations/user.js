...
export const updateUser = addUser.append().keys({
  ...
  osType: Joi.string().valid('android', 'ios').error(new Error('Valid os type is required (android / ios)')).optional(),
  firstName: Joi.string().min(3).max(100).error(new Error('First name is required')).optional(),
  lastName: Joi.string().min(3).max(100).error(new Error('Last name is required')).optional(),
  email: Joi.string().email().error(new Error('Valid email id is required')).optional(),
  address: address.append(),
});

export const socialAuth = Joi.object().keys({
  fullName: Joi.string().min(3).max(100).error(new Error('Min 3 and max 100 letters are allowed in full name')),
  profilePic: Joi.string().allow(''),
  email: Joi.string().email(),
  socialType: Joi.string(),
  socialId: Joi.string(),
  deviceToken: Joi.string(),
  osType: Joi.string().valid('android', 'ios').error(new Error('Valid os type is required (android / ios)')),
  role: Joi.string().valid('user','engineer').required().error(new Error('User role could be "user" or "engineer"')),
});
...
export const availableEngineers = pagination.append().keys({
...
export const addReviews = Joi.object().keys({
  engineerId: Joi.string().guid().required().error(new Error('engineerId is required')),
  ...
  rating: Joi.number().positive().min(0).max(5).required().error(new Error('rating is required')),
  review: Joi.string().optional(),
  type: Joi.string().required().valid('feedback', 'dispute').error(new Error('type: `feedback` | `dispute` is required')),
});
